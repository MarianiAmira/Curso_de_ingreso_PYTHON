...
class App(customtkinter.CTk):
    ...
    def btn_convertir_c_f_on_click(self):
        temperatura_txt = self.txt_temperatura_c.get()
        temperatura_int = int(temperatura_txt)
        resultado_int =  (temperatura_int - 32) * 5/9
        resultado_txt = str(resultado_int)
        mensaje = "La temperatura en Centígrados es : {0}".format(resultado_txt)
        alert(title="Resultado", message=mensaje)

    def btn_convertir_f_c_on_click(self):
        centigrados_txt = self.txt_temperatura_f.get()
        centigrados_int = int(centigrados_txt)
        resultado_int = centigrados_int *1.8 +32
        resultado_txt = str(resultado_int)
        mensaje = "La temperatura en Fahrenheit es : {0}".format(resultado_txt)
        alert(title="Resultado", message=mensaje)
