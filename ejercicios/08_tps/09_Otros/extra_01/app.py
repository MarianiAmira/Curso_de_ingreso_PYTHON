...
class App(customtkinter.CTk):
    ...
    def btn_agregar_on_click(self):
        ...
        while respuesta != None:
            nombre_articulo = prompt(title="Producto", prompt="Ingrese el producto ('TV', 'LICUADORA')")
            while nombre_articulo != "TV" and nombre_articulo != "LICUADORA":
                alert(title="ERROR", message="ERROR")
                nombre_articulo = prompt(title="Producto", prompt="Ingrese el producto ('TV', 'LICUADORA')")

            precio_articulo = int(prompt(title="Precio", prompt="Ingrese el precio del producto (entre 200 y 1000"))
            while precio_articulo < 200 or precio_articulo > 1000:
                alert(title="ERROR", message="ERROR")
                precio_articulo = int(prompt(title="Precio", prompt="Ingrese el precio del producto (entre 200 y 1000"))

            respuesta = prompt(title="Pregunta", prompt="Desea ingresar otro producto?")
            
        self.lista_nombre_articulo.append(nombre_articulo)
        self.lista_precio_articulo.append(precio_articulo)

        alert(title="Dato", message="Se cargo correctamente")


    def btn_mostrar_on_click(self):
        for productos in range(len(self.lista_nombre_articulo)):
            #producto = self.lista_nombre_articulo
            #precio = self.lista_precio_articulo
            print(f"{productos + 1}. Producto: {productos}")
